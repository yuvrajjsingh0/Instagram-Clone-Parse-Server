...
public class ImageProcessor {
    ...
    public Bitmap boost(Bitmap originalImage, int type, double percent) {
        ...
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
                ...
                B = Color.blue(pixel);
                switch (type) {
                    case ImgConstants.RED:
                        R = (int) (R * (1 + percent));
                        if (R > 255) R = 255;
                        break;
                    case ImgConstants.GREEN:
                        G = (int) (G * (1 + percent));
                        if (G > 255) G = 255;
                        break;
                    case ImgConstants.BLUE:
                        B = (int) (B * (1 + percent));
                        if (B > 255) B = 255;
                        break;
                }
    ...
    public Bitmap flip(Bitmap originalImage, int type) {
        Matrix matrix = new Matrix();
        if (type == ImgConstants.FLIP_VERTICAL) {
            matrix.preScale(1.0f, -1.0f);
        } else if (type == ImgConstants.FLIP_HORIZONTAL) {
            matrix.preScale(-1.0f, 1.0f);
        } else {
            return null;
        }
    ...
    public Bitmap tintImage(Bitmap originalImage, int degree) {
        ...
        double angle = (ImgConstants.PI * (double) degree)
                / ImgConstants.HALF_CIRCLE_DEGREE;
        int S = (int) (ImgConstants.RANGE * Math.sin(angle));
        int C = (int) (ImgConstants.RANGE * Math.cos(angle));
        for (int y = 0; y < height; y++)
            for (int x = 0; x < width; x++) {
                int index = y * width + x;
    ...
    public Bitmap applyFleaEffect(Bitmap originalImage) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                index = y * width + x;
                int randColor = Color.rgb(random.nextInt(ImgConstants.COLOR_MAX),
                        random.nextInt(ImgConstants.COLOR_MAX),
                        random.nextInt(ImgConstants.COLOR_MAX));
    ...
    public Bitmap applyBlackFilter(Bitmap originalImage) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                thresHold = random.nextInt(ImgConstants.COLOR_MAX);
                if (R < thresHold && G < thresHold && B < thresHold) {
                    pixels[index] =
                            Color.rgb(ImgConstants.COLOR_MIN, ImgConstants.COLOR_MIN,
                                    ImgConstants.COLOR_MIN);
                }
            }
        }
    ...
    public Bitmap applySnowEffect(Bitmap originalImage) {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                if (R > thresHold && G > thresHold && B > thresHold) {
                    pixels[index] =
                            Color.rgb(ImgConstants.COLOR_MAX, ImgConstants.COLOR_MAX,
                                    ImgConstants.COLOR_MAX);
                }
            }
        }
